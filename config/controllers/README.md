...
says which platforms are played with it, which danstick layout a capture walks,
...
```yaml
label: GameCube
layout: gamecube          # the danstick layout the capture walks
...
controls:                 # danstick's control id -> what the button says
  a: A (large centre)
...
## Why `controls` repeats danstick

Because three things describe a GameCube pad — danstick's layout, this file, and
...
against danstick's own layouts whenever danstick is checked out beside GOTG.

## `a: B` is not a typo

danstick names a control by where it sits on a modern pad: `a` is the bottom face
...
1. Write the file. `layout` has to be a layout danstick actually has —
