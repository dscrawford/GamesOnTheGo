...
danstick grabs a physical pad and republishes it as "danstick Player N", and the
...
def test_a_clone_is_dansticks_by_name():
    assert clones.is_clone("danstick Player 1")
    assert clones.is_clone("danstick Player 4")


def test_a_clone_is_dansticks_by_phys_when_the_name_was_not_kept():
    # danstick sets phys best-effort and falls back to the name; a front-end
    # that had only the phys should still know its own.
    assert clones.is_clone("Xbox 360 Controller", "danstick/p2")
...
def test_a_published_pad_drives_and_the_pad_it_was_made_from_does_not():
    # Both are open at once, which is the whole difficulty: danstick grabs the
    # first and publishes the second, and only the second has a seat.
    owners = opened(p3="Microsoft X-Box 360 pad", p7="danstick Player 1")
...
def test_a_pad_nobody_opened_drives_nothing():
    ...
    assert not opened(p1="danstick Player 1").may_drive(9)
    assert not opened(p1="danstick Player 1").may_drive(None)
...
def test_an_unplugged_pad_is_forgotten_rather_than_left_behind():
    ...
    owners = opened(p4="danstick Player 2")
    owners.closed(4)
    assert not owners.may_drive(4)


def test_a_pad_opened_before_danstick_was_anywhere_still_does_not_drive():
    # Nothing about the daemon's state reaches this: the pad is either one
    # danstick published, or it is not.
    owners = opened(p3="Microsoft X-Box 360 pad", p7="danstick Player 1")
    assert not owners.may_drive(3)
    assert owners.may_drive(7)
...
# pad seated through danstick's seating mode.

CLONE_OF_AN_UNKNOWN_PAD = "0300324caa2a0000bb5b000002000000"   # "danstick Player 2"
CLONE_OF_AN_XBOX_PAD = "0300f38c5e0400008e02000003000000"      # "danstick Player 3"
...
def test_sdl_renames_a_clone_of_a_pad_it_knows_and_the_guid_still_says():
    # The bug this exists for: danstick's clone mirrors its source's vendor and
    # product by default, so SDL looks 045e:028e up in its own database and
    # calls the clone "Xbox 360 Controller" -- the kernel's "danstick Player 3"
...
def test_a_clone_of_a_pad_sdl_does_not_know_keeps_its_name_and_agrees():
    assert clones.is_clone("danstick Player 2", guid=CLONE_OF_AN_UNKNOWN_PAD)
...
def test_a_guid_that_is_not_one_decides_nothing():
    ...
    assert clones.is_clone("danstick Player 1", guid="nonsense")


def test_a_renamed_clone_drives_the_picker():
    # The failure this would otherwise be: a player seats themselves, danstick
...
def test_a_clone_says_which_seat_it_is():
    ...
    """
    assert clones.player_of("danstick Player 3") == 3
    assert clones.player_of("Xbox 360 Controller", "danstick/p2") == 2
