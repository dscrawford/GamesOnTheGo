...
# the picker hearing danstick. Steam hands every launch the controllers Steam
# Input handles, to be ignored in favour of its virtual gamepad -- and danstick's
...
# (danstick_clear_steam_env); the picker never goes through it.
...
# SDL's HIDAPI backend claims Valve's ids and then hides danstick's evdev clone
...
# danstick owns the hidraw device, and its clone is the one SDL should see.
# Before pygame initialises anything, which is why it lives here.
_os.environ.setdefault("SDL_JOYSTICK_HIDAPI", "0")

# danstick's own SDL mappings for the pads it has published, so a clone SDL
# knows nothing about -- a Steam Controller's -- opens as a gamepad with
# danstick's button names rather than as a bare joystick in an Xbox pad's
...
# lines from `danstick-rs exec`.
_mappings = _os.path.join(
    _os.environ.get("XDG_CONFIG_HOME") or _os.path.expanduser("~/.config"), "danstick", "sdl_controllers.txt"
)
