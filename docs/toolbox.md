...
vocabulary: `verifySfv`, `unrar`, `extract7z`, `unzip`, `pickLargest`, and the
terminal steps `placeTree`, `keepExtension`, `convertRvz`. Two canned chains
live in `helpers.nix` — `sceneArchiveRecipe` and `discArchiveRecipe`.
